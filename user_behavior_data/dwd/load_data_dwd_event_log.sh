...
select 
	...
	lat,
	get_json_object(event_json,'$.kv.action') action,
	get_json_object(event_json,'$.kv.goodsid') productid,
	get_json_object(event_json,'$.kv.place') order,
	get_json_object(event_json,'$.kv.extend1') extend1,
...
insert overwrite table "$APP".dwd_product_detail_log
PARTITION (dt='$do_date')
select 
	mid_id,
	user_id,
	...
	get_json_object(event_json,'$.kv.news_staytime') product_staytime,
	get_json_object(event_json,'$.kv.loading_time') loading_time,
	get_json_object(event_json,'$.kv.type1') type1,
	get_json_object(event_json,'$.kv.category') category,
	server_time
...
select 
	...
	get_json_object(event_json,'$.kv.newstype') adtype,
...
insert overwrite table "$APP".dwd_frontend_act_log
PARTITION (dt='$do_date')
select 
	mid_id,
	user_id,
...
insert overwrite table "$APP".dwd_backend_act_log
PARTITION (dt='$do_date')
select 
	mid_id,
	user_id,
...
select 
	...
	get_json_object(event_json,'$.kv.praise_count') like_count,
...
insert overwrite table "$APP".dwd_like_log
PARTITION (dt='$do_date')
select 
	mid_id,
	user_id,
